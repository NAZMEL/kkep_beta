    ...
    if(isset($_GET['enter_search']) && (!empty($_GET['search']))){
     ...
     if($news[$i]['full_text'] != ""){
         ?>
                    ...
                    <div id="main_news" style="margin-left: 10px; font-size: 21px;line-height:25px; text-indent: 1cm; "><?=substr($news[$i]['full_text'], 0, 3000).'...';?><a style="color: grey; font-style: italic;" href="/templates/main_content/artickle.php?id=<?= $news[$i]['id']?>"> читати далі.</a></div> 
     ...
     ?>
            ...
            <hr style="position: relative; color: grey; border-style: dashed;"/>
             ...
             <span title="Порокоментували" style="position: absolute; font-size: 20px; margin-left: 100px;"><img src="/images/Koment.png" alt="Прокоментували" id ="coment" style=" width: 25px; height: 25px; "></span>&nbsp<span title="Порокоментували" id="koment" style="margin-left: 125px; font-size: 20px;"><a href="/templates/main_content/artickle.php?id=<?=$news[$i]['id'];?>#comment_style"><?= $news[$i]["comments"];?></a></span>
